...
**Then regenerate the printed graphics — from the Dashboard.** `scripts/card.js`
and `scripts/sticker.js` read `process.env.BASE_URL` directly, and `dotenv` is
loaded only by `server/config.js`, which neither of them requires. Run as
`npm run card` or `npm run sticker` from a shell they see no `BASE_URL` and
emit a placeholder without failing. **Settings → Graphics** runs the same code
inside the server process, which read `.env` at boot. The business card and the sticker
...
which matches the reason the VPS is in Beauharnois. At two emails an order it
leaves a wide margin. Resend, Postmark and Brevo also have free tiers; Fastmail
and Migadu are the cheap paid ones and give a real mailbox with IMAP too.

Cyberimpact validates the domain with **three CNAMEs** — two DKIM and one for
SPF/Return-Path — added on subdomains, so the root record below is left alone.
Add them at Cloudflare **DNS-only (grey cloud)**; proxying them breaks DKIM.
Use the provider's own Copy button, because a wrong character fails validation
...
| QR code goes somewhere wrong | Generated before `BASE_URL` was set, or generated by `npm run card` / `npm run sticker` from a shell, which never reads `.env`. Regenerate in Settings → Graphics. The card prints its address under the code; the sticker does not, so check a sticker by scanning it. |
