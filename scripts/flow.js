...
(async () => {
  ...
  /* --- Anonymous ------------------------------------------------------- */
  {
    ...
    /* --- and the manifest answers anyway ---------------------------------
       Asked with no session ON PURPOSE, and noCookie is the whole check. A
       <link rel="manifest"> is fetched with credentials omitted, so the
       browser's own request for this file carries no cookie no matter who is
       signed in. Guarded like the export above it, this would answer 401, the
       browser would decline to offer the install, and it would say so nowhere
       — passing every check a signed-in owner could run by hand and failing
       only on the phone it was for. */
    const adminManifest = await GET('/admin/manifest.webmanifest', { noCookie: true });
    check('the dashboard manifest answers a browser carrying no session', adminManifest.status, 200);
    const dm = JSON.parse(adminManifest.text);
    check('it opens on the dashboard', dm.start_url, '/admin');
    ok('and its scope contains where it opens', dm.start_url.startsWith(dm.scope));
    check('it opens in its own window', dm.display, 'standalone');
    check('and stays out of the customer app by carrying its own id', dm.id, '/admin');
    ok('it offers both sizes a browser insists on before it will install',
      ['192x192', '512x512'].every((size) => dm.icons.some((i) => i.sizes === size)));
    ok('and offers a maskable one, so Android does not crop the mark',
      dm.icons.some((i) => i.purpose === 'maskable'));
    ok('the icons are the dashboard set, not the customer one',
      dm.icons.every((i) => i.src.startsWith('/icons/admin-')));

    /* A manifest naming an icon that 404s is a manifest no browser installs,
       and the icons are generated by a script nobody runs on a schedule. */
    for (const i of dm.icons) {
      const art = await GET(i.src, { noCookie: true });
      check(`the icon ${i.src.replace('/icons/', '')} is really there`, art.status, 200);
    }
    const apple = await GET('/icons/admin-apple-touch-icon.png', { noCookie: true });
    check('and so is the one iOS takes the home screen picture from', apple.status, 200);

    /* --- and each one is cut for the platform that will crop it -----------
       The icons are generated by `npm run icons` and committed, so nothing
       regenerates them on the way to a phone. Each platform cuts a different
       shape out of the same square, and getting it wrong is invisible until it
       is on somebody's home screen. */
    const sharp = require('sharp');
    const icons = path.join(__dirname, '..', 'public', 'icons');
    const pixels = async (f) => {
      const { data, info } = await sharp(path.join(icons, f)).ensureAlpha().raw()
        .toBuffer({ resolveWithObject: true });
      return { data, W: info.width, H: info.height, ch: info.channels };
    };

    /* The plain pair is what a desktop browser shows, and the only place the
       rounded corners are ever seen. Transparent, not a colour: the tile is
       drawn on backgrounds this repo cannot know. */
    for (const f of ['admin-icon-192.png', 'admin-icon-512.png']) {
      check(`${f} has its corners rounded away`, (await pixels(f)).data[3], 0);
    }

    /* iOS masks to its own squircle and fills transparency with black; Android
       crops the maskable pair to a circle. Both need a real pixel everywhere. */
    for (const f of ['admin-apple-touch-icon.png', 'admin-icon-maskable-192.png',
      'admin-icon-maskable-512.png']) {
      check(`${f} is full bleed, so the platform can cut its own shape`,
        (await pixels(f)).data[3], 255);
    }

    /* The one that matters on an Android launcher, and the one a later change
       to the layout would quietly break: everything drawn has to sit inside the
       circle Android 12+ crops to — two thirds of the canvas — measured on the
       diagonal of the whole arrangement rather than on its height. */
    const mask = await pixels('admin-icon-maskable-512.png');
    const mx = mask.W / 2, my = mask.H / 2;
    const ground = [mask.data[0], mask.data[1], mask.data[2]];
    let furthest = 0;
    for (let y = 0; y < mask.H; y++) {
      for (let x = 0; x < mask.W; x++) {
        const i = (y * mask.W + x) * mask.ch;
        const off = Math.abs(mask.data[i] - ground[0]) + Math.abs(mask.data[i + 1] - ground[1])
          + Math.abs(mask.data[i + 2] - ground[2]);
        if (off < 60) continue;                        // still the olive ground
        const r = Math.hypot(x - mx, y - my);
        if (r > furthest) furthest = r;
      }
    }
    ok('nothing on the maskable icon falls outside the crop Android applies',
      furthest <= 170, `artwork reaches ${Math.round(furthest)}px from centre, of 170`);
  ...
  /* --- Sign in ---------------------------------------------------------- */
  {
    ...
    /* The five lines that make it installable, on the page as served. iOS
       reads the manifest for the name but takes the picture from
       apple-touch-icon, and with no such tag on an admin page it falls back to
       a screenshot of whatever was on screen. */
    ok('the dashboard points at its own manifest',
      dash.text.includes('rel="manifest" href="/admin/manifest.webmanifest"'));
    ok('and at an icon iOS can use instead of a screenshot',
      dash.text.includes('/icons/admin-apple-touch-icon.png'));
    ok('and names itself Dashboard under the icon',
      dash.text.includes('name="apple-mobile-web-app-title" content="Dashboard"'));

    /* The login page carries them too: signing in is not a prerequisite for
       adding an icon to a home screen, and the login page is where somebody
       who has not signed in on that device is standing. */
    const signin = await GET('/admin/login', { noCookie: true });
    ok('the login page is installable on its own',
      signin.text.includes('rel="manifest" href="/admin/manifest.webmanifest"'));

    /* And both carry somewhere to put the offer. Chrome fires
       beforeinstallprompt and then says nothing, so without this the install
       lives behind a menu nobody opens — which is how it sat until somebody
       asked where the button was. */
    ok('the dashboard has somewhere to show the install offer',
      dash.text.includes('id="install-slot"'));
    ok('and so does the login page', signin.text.includes('id="install-slot"'));
    ok('the login page loads the script that fills it',
      signin.text.includes('/install.js?v='));
